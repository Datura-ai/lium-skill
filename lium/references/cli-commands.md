...
| 4 | SSH error |
| 3 | API error — the API refused the key (401); today only `lium audit` maps it *(lium#160, not released)* |
| 5 | Pod not found |
...
Code 6 exists in the source but no command produces it today; code 3 is produced only by `lium audit` (lium#160, not released) — no released command exits 3.
