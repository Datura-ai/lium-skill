...
lium ls [OPTIONS]
  ...
  --nvlink                Only nodes whose GPUs are all joined by NVLink (Link column NV#);
                          nodes with no topology report yet are excluded  (since 0.4.0)
...
lium ls --gpu H200 --nvlink --min-download 2000   # NVLink boards with a Download floor — since 0.4.0
```
...
`--json`. On a CLI older than 0.4.0, verify on the pod (`nvidia-smi topo -m`, `nvidia-smi topo -p2p r`,
a timed download).
...
lium up [OPTIONS] [NODE_ID]
  NODE_ID                     Node UUID, HUID, or index from the last `lium ls`.
                              ...
                              Pass the UUID (`id` in `lium ls --format json`) on any
                              release before 0.3.0: there the HUID answers
                              "Node '<huid>' not found" although the help lists it.
                              Since 0.3.0 (lium#153) the HUID resolves too.
...
An API key needs the read scope to read `GET /users/me/events`.
...
```bash
lium fund [OPTIONS]
  -w, --wallet TEXT  Name of the local wallet to fund from
...
| Index from the last `lium ps` | `lium ssh 1` | `ssh`, `exec`, `scp`, `rsync`, `rm`, `reboot`, `update`, `port-forward`, `bk *`, `audit --pod` (since 0.0.37) — **not** `ps`, `describe` or `logs` |
