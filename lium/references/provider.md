...
|---|---|
| `LIUM_OUTPUT=json` | Same as `--json`: one JSON envelope per command. On every `lium provider` command and on `lium mine` *(coming with the next CLI release)*. |
...
- `lium provider …` prints **one envelope on stdout**. Success is `{"ok": true, "data": …}`. Failure is
  `{"ok": false, "error": {"code", "message", "hint", "exit_code", "data"?, "request_id"?}}`, and the process exits
  with `exit_code`. Progress, when a command has any, goes to stderr as one JSON object per line.
- `--json` and `--yes` work after the subcommand (`lium provider node listing --json`) or on the group
  (`lium provider --json node listing`).
- Every command that changes something asks the persona gate once per user. Off a terminal or under `--json` it
  never prompts: it fails with `input.confirmation_required` (exit 2). Pass `--yes` or set `LIUM_PROVIDER_ACK=1`.
- Branch on `error.code`, not on `message`. Codes are namespaced snake_case and are never renamed once shipped.
  The old UPPER_CASE code stays in `error.legacy_code` *(coming with the next CLI release)*.
...
For each gating reason:

1. If `requires` contains `reboot`, or `sudo` on a host you cannot `sudo` on, **stop**: it is a
   [human step](#reboots-and-other-host-steps).
2. If `requires` contains `no_rentals`, the fix interrupts a rental: pause new rentals first and wait until the
   current rental ends (`lium provider node pause "$NODE" --json --yes`, *coming with the next CLI release*).
3. Run `fix_command` when the reason has one; otherwise do what `fix` says. Run `verify_command` when present.
...
Branch on `error.code`; the exit code is the coarse class.
...
| 2 | `input.confirmation_required`, `input.input_required`, `input.register_token_invalid`, `input.hotkey_conflicts_with_token` | A value or a confirmation nobody could give | Add what `error.hint` names (`--yes` or `LIUM_PROVIDER_ACK=1`, `-k`, `LIUM_PROVIDER_PASSWORD`, a new register token) and re-run once. Never retry unchanged. |
| 3 | `portal.<detail.code>`, `portal.not_supported` | The portal refused or failed the call | `portal.not_supported`: the portal does not serve this route yet, so stop using the command. Other codes: act on `error.message`/`error.data`; a 5xx may be retried up to 3 times with backoff. |
| 4 | `net.unreachable`, `ssh.*` | Nothing answered | Check the network and `--portal-url`; retry with backoff (30 s, 60 s, 120 s). |
| 5 | `node.not_found`, `portal.executor_not_found` | The node is not on this account | Re-read the ids with `lium provider node listing --json`. |
| 6 | `auth.*` | Signed out, expired or not allowed | Sign in again (`lium provider portal login --force --json`) or use a token with the right scope. Do not loop. |
| 7 | a portal 429 (`PORTAL_RATE_LIMIT` today) | Rate limit or a token-cache race | Wait 60 s and retry. |
...
| 12 | `human.*` | A person has to act | Relay the message ([One-time human steps](#one-time-human-steps)), wait for the person, then re-run the same command. |

Exits 10 and 11 come with the next CLI release; 12 is reserved for the human steps below.
Old-style failures still carry an UPPER_CASE code with the old provider exit statuses: 1 argument, 2 auth, 3 portal,
5 SSH, 6 config missing, 7 token-cache race. So an exit 2 can be an auth failure. On 0.9.1 every provider failure is
old-style; the full old-to-new table is in the CLI's `docs/exit-codes.md`.
...
|---|---|---|---|
| `nvidia_driver_below_minimum` | The NVIDIA driver is older than the network minimum (580.65.06 today) | sudo, reboot | Human step: upgrade the driver, reboot, restart the executor (`docker compose up -d` in `neurons/executor`). |
| `sysbox_not_enabled` | The executor does not run the sysbox runtime | sudo, no_rentals | Install and enable sysbox, restart the executor. |
| `insufficient_disk_for_vram` | Total disk is below the required share of GPU VRAM (`measured` vs `required`, in GB) | sudo | Give the host at least `required` GB of disk. New hardware is a human step. |
| `flagship_without_ncu_or_split` | An idle 8x flagship offers no NCU profiling, GPU splitting or confidential computing | sudo, reboot (profiling path only) | No reboot: `lium provider node min-gpu set "$NODE" <n> --json --yes` with `n` below the full node. Otherwise open the profiling counters (`NVreg_RestrictProfilingToAdminUsers=0`, then reboot) as a human step, or run in a confidential VM. |
| `cannot_apply_gpu_power_cap` | The executor container cannot run `nvidia-smi -pl` | sudo | `docker compose pull && docker compose up -d` in `neurons/executor`; a custom compose needs `privileged: true`. |
...
| `banned_network_abuse` | The node is banned for network abuse | human | Human step: the owner contacts Lium support. Do not retry. |
...
**Reachability** (`kind: availability`, the listing's `hidden_reasons`) always blocks renting:

| `code` | Meaning | `requires` | Fix, then verify |
|---|---|---|---|
| `NOT_RESPONDING` | The validator cannot reach the executor | sudo | Start the executor (`docker compose up -d` in `neurons/executor`) and open its port to the internet. |
| `NOT_ACTIVE`, `NOT_VERIFIED` | The node has not passed a validator check yet | none | Wait for the next check, or fix the error `node status` reports. |
| `DISK_TOO_FULL` | Disk more than 90% used | sudo | Free disk until at most 90% is used. |
| `DISK_FREE_TOO_LOW` | Too little free disk | sudo | Free disk or add a larger disk. |
| `DISK_NOT_REPORTED` | The executor does not report its disk | sudo | Update and restart the executor. |
| `NETWORK_TOO_SLOW` | The uplink is below the minimum | human | Human step: move the node to a faster uplink. |
| `GPU_COUNT_UNKNOWN` | The validator could not read the GPUs | sudo | Restart the executor. |
...
**The validator's last error** (`kind: last_error`) always blocks renting. `code` is the validator's reason code;
do what `fix` says (the validator's own remediation), then verify.
...
answer, then run the command under "After". When a command answers exit 12 (`human.*`), relay
`error.data.message_for_human` verbatim when it is there, else `error.message`. The planned handoff form of that
answer is `{"code": "human.handoff_required", "exit_code": 12, "data": {"step", "handoff_url", "code", "expires_at",
"message_for_human"}}`; no CLI release or open CLI pull request returns it yet.
...
Relay, with `<url>` = `data.authorization_url`:

> To get idle pay for your Lium provider nodes, link your Discord account once: open <url> in a browser, sign in to
> Discord and approve Lium. Tell me when you are done.

After, poll every 30 s for up to 10 minutes until `data.discord_connected` is `true`, then verify the node:
...
An e-mail account must be confirmed before `portal login --email` works; the refusal names it in `error.message`.
The CLI has no command to confirm it. Relay:
...
A reason whose `requires` contains `reboot`, a fix that needs `sudo` you do not have, new hardware, a faster uplink or
a support ticket is a person's step. Do not reboot on your own: a reboot ends any rental on the node. Relay:

> Node <node_id> on <host> needs you for one step: <fix>. I stopped here because it needs <a reboot | root access |
> new hardware | Lium support>. Please do it, then tell me when the machine is back up.
