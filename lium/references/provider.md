...
- Branch on `error.code`, not on `message`. From the next CLI release codes are namespaced snake_case and never
  renamed once shipped, and the old UPPER_CASE code stays in `error.legacy_code`.
...
- `requires` is missing or empty (the CLI built the list itself, `"blocking_reasons_source": "cli_fallback"`) and
  `fix` mentions a reboot, a restart, stopping Docker or the executor, or reformatting a disk. Also use the
  `requires` column of [Blocking reasons](#blocking-reasons) for that code;
...
| 7 | `portal.rate_limited` (a portal 429) | Rate limit or a token-cache race | Wait 60 s and retry. |
...
| 12 | `human.handoff_required`, `human.handoff_expired` | A person has to act | Relay `error.data.message_for_human` ([One-time human steps](#one-time-human-steps)), wait for the person, then re-run. |
...
A reason's `requires` comes with the reason and wins over this table; when it is missing or empty, use this column.
It names what the fix needs: `sudo` (root on the GPU host), `reboot` and `no_rentals` (both a
[hand-over](#5-fix-then-verify)).
...
| `flagship_without_ncu_or_split` | An idle 8x flagship offers no NCU profiling, GPU splitting or confidential computing | no_rentals, reboot (profiling path) | Yours, no reboot: `lium provider node min-gpu set "$NODE" <n> --json --yes` with `n` below the full node. Otherwise hand over: open the profiling counters (`NVreg_RestrictProfilingToAdminUsers=0`, then reboot), or run in a confidential VM. |
| `cannot_apply_gpu_power_cap` | The executor container cannot run `nvidia-smi -pl` | sudo, no_rentals | Hand over: `docker compose pull && docker compose up -d` in `neurons/executor`; a custom compose needs `privileged: true`. |
| `outdated_executor_image` | The executor image is not the current release | sudo | Do what the reason's `fix` says (the validator's text): on a standard stack, check that `executor-executor-runner-1` and `executor-watchtower-1` are running (`docker ps`) so Watchtower redeploys the current image; if auto-update stopped, hand over with the `docs_url` (https://docs.lium.io/providers/nodes/gpu-power-cap#the-standard-stack-which-stopped-updating). |
...
| `DISK_TOO_FULL` | Disk more than 90% used | sudo | Free disk until at most 90% is used, within the limits below. |
| `DISK_FREE_TOO_LOW` | Too little free disk | sudo | Free disk within the limits below; a larger disk is a hand-over. |
| `DISK_NOT_REPORTED` | The executor does not report its disk | sudo | Hand over (restarts the executor): update and restart the executor. |
| `NETWORK_TOO_SLOW` | The uplink is below the minimum | human | Human step: move the node to a faster uplink. |
| `GPU_COUNT_UNKNOWN` | The validator could not read the GPUs | sudo | Hand over (restarts the executor): restart the executor. |
...
*(coming with the next CLI release)* The CLI asks the portal for a handoff: one URL plus a short code. The command
exits 12 with `{"ok": false, "error": {"code": "human.handoff_required", "exit_code": 12, "data": {"step",
...
`--timeout` passes first. A portal without handoff sessions answers exit 3, `portal.not_supported`, with
`data.legacy_flow: true` and `data.legacy_browser_url`: that link is the one-time step, see each section.
...
Every fix the [stop rule](#5-fix-then-verify) names is a person's step: `requires` with `reboot` or `no_rentals`, a
fix text (with no `requires`) that reboots, restarts, stops Docker or the executor, or reformats, `sudo` you do not
have, new hardware, a faster uplink or a support ticket. Never reboot unattended. Relay:

> Node <node_id> on <host> needs you for one step: <fix>. I stopped here because it needs <a reboot | an interruption
> of rentals | root access | new hardware | Lium support>. Please do it, then tell me when the machine is back up.
