...
- **Provider journey**: [references/provider.md](references/provider.md) (also at https://raw.githubusercontent.com/Datura-ai/lium-skill/main/lium/references/provider.md) — running the owner's GPU machines as Lium provider nodes unattended: `lium mine`, `lium provider` with `--json`/`--yes`, the exit and error codes, blocking reasons and their fixes, and the one-time steps that need a person.
