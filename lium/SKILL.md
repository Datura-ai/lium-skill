...
curl -fsS -X POST $API/auth/signup -H 'Content-Type: application/json' -d '{}' > account.json
export LIUM_API_KEY=$(jq -r .api_key account.json)
# 2. A key that can only pay (session login 0.4 s + mint 0.4 s). It reaches no pods.
TOKEN=$(curl -fsS -X POST $API/auth/login -H 'Content-Type: application/json' \
  -d "{\"fingerprint\":\"$(jq -r .fingerprint account.json)\"}" | jq -r .token)
BILLING_KEY=$(curl -fsS -X POST $API/keys -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"name":"agent-billing","scopes":["billing"]}' | jq -r .key)
# 3a. Card: a Stripe page for a person to pay (0.5 s; nothing is charged until they pay)
curl -fsS -X POST $API/stripe/create-checkout-session -H "X-API-KEY: $BILLING_KEY" -H 'Content-Type: application/json' \
...
# curl -fsS -X POST $API/tmc-pay/create-invoice -H "X-API-KEY: $BILLING_KEY" -H 'Content-Type: application/json' \
...
until [ "$(curl -fsS $API/users/me -H "X-API-KEY: $LIUM_API_KEY" | jq '.balance > 0')" = true ]; do sleep 2; done
# 5. Rent the cheapest matching node; returns when SSH answers (about 20-26 s on a cached image)
POD=$(lium up --gpu RTX4090 --ttl 30m --yes --json | jq -r .pod.huid)
lium exec "$POD" --json "nvidia-smi -L"
```

With the next CLI release (lium#296) steps 1–4 are two commands:

```bash
lium signup --no-email --billing-key --json > account.json   # account, rent key, billing key, SSH key: ~1 s
lium topup link -a 10 --wait 900 --json 2>handoff.json       # handoff.json has the page for the payer; exits once credited
```
...
5/min, 20/day. The released CLI has no flag for it yet (`lium signup --no-email` comes with lium#296); today `lium signup` is the email path only. Full walkthrough:
...
escaped, like every `$` inside a double-quoted `lium exec "…"`, or the local shell expands it to nothing and the
pod gets an empty PID file) and stop with `kill \$(cat /tmp/job.pid)` as below. If you must search by name, list first and
...
# 1. Pick hardware from JSON, not from the table, and keep the node's id (the UUID) for step 2.
#    Pick from ls yourself only when you need a filter `up --gpu` lacks (here: tier and a price sort).
#    Otherwise rent by spec (`lium up --gpu H200 -c 8`, see Non-Interactive Pod Creation): since 0.0.39 the backend
#    picks the cheapest node; 0.0.37/0.0.38 pick the first Pareto row locally. A node you pick here may be
#    taken by step 2 — the `||` cleanup there covers that.
NODE=$(lium ls --gpu H200 --count 8 --format json | jq -c 'map(select(.tier=="secure")) | sort_by(.price_per_gpu_hour) | .[0]')
[ "$NODE" != null ] || { echo "no secure-tier 8×H200 node is listed"; exit 1; }
jq -r '"\(.huid) \(.config) $\(.price_per_hour)/h \(.country)"' <<<"$NODE"
NODE_ID=$(jq -r '.id' <<<"$NODE")

# 2. Rent that node, non-interactively, with a TTL; --no-ssh returns as soon as the pod is RUNNING.
#    A node id takes no --gpu/-c (a node id and the filters exclude each other); with no --count the
#    rent takes the node's free GPUs — the 8 that step 1 filtered on — and --verify-gpus --strict-gpus
#    removes the pod when the GPU count it sees is not that.
...
lium up "$NODE_ID" --name "$NAME" --ttl 6h -y --no-ssh --verify-gpus --strict-gpus \
...
- A bad or revoked key exits `3` — prove the key with `lium balance` (see No User Identity Command). A 403 on rent exits `6` and means Insufficient balance, not a bad key (see Before the First Rental — Balance).
