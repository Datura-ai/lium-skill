...
`lium rm <pod>` and pick another node. If the job must run there anyway, NCCL only works
over loopback sockets, and TP=8 serving of a large model is impractical:

```bash
export NCCL_P2P_DISABLE=1 NCCL_SHM_DISABLE=1
export NCCL_IB_DISABLE=1 NCCL_SOCKET_IFNAME=lo   # virtualised host without RDMA NICs
```
...
```bash
lium exec <pod> "curl -o /dev/null -sS -w '%{speed_download}\n' 'https://speed.cloudflare.com/__down?bytes=500000000'"   # bytes/s
lium exec <pod> "cd /workspace && time hf download openai-community/gpt2 model.safetensors --local-dir /workspace/probe"
```
